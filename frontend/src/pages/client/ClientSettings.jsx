...
function Toggle({ checked, onChange }) {
  return (
    <button onClick={() => onChange(!checked)}
      ...
      style={{ background: checked ? BLUE : "#d1d5db" }}>
...
export default function ClientSettings() {
  ...
  const [notifs, setNotifs] = useState({
    emailWorkoutPlan: true,
    emailMealPlan: true,
    emailSubscriptionUpdate: true,
    emailChatMessage: false,
    pushNewMessage: true,
    pushPlanUpdate: true,
  ...
  const [privacy, setPrivacy] = useState({
    ...
    showBmiToTrainer: true,
    allowReviewVisibility: true,
  });

  useEffect(() => {
    api.get("/api/profile/me")
      .then(res => { setName(res.data.name || ""); setEmail(res.data.email || ""); setLoadingAccount(false); })
      .catch(() => { setName(localStorage.getItem("name") || ""); setLoadingAccount(false); });

    const savedNotifs  = localStorage.getItem("fittrack_notif_prefs");
    if (savedNotifs)  setNotifs(JSON.parse(savedNotifs));
    const savedPrivacy = localStorage.getItem("fittrack_privacy_prefs");
  ...
  const saveAccount = async () => {
    ...
    setSaving(true);
    try {
      await api.put("/api/profile/update-name", { name: name.trim() });
  ...
  const saveNotifications = async () => {
    setSaving(true);
    localStorage.setItem("fittrack_notif_prefs", JSON.stringify(notifs));
  ...
  const savePrivacy = async () => {
    setSaving(true);
    localStorage.setItem("fittrack_privacy_prefs", JSON.stringify(privacy));
    await new Promise(r => setTimeout(r, 400));
  ...
  const logout = () => {
    Swal.fire({ title: "Log out?", icon: "question", showCancelButton: true, confirmButtonColor: BLUE, cancelButtonColor: "#6b7280", confirmButtonText: "Yes, log out" })
  ...
  const deleteAccount = () => {
    Swal.fire({
      ...
      html: "This will permanently delete your account, progress logs, and subscription data.<br><br><b>This cannot be undone.</b>",
      icon: "warning", showCancelButton: true, confirmButtonColor: "#dc2626", cancelButtonColor: "#6b7280", confirmButtonText: "Yes, delete everything",
    }).then(async r => {
      if (r.isConfirmed) {
        try { await api.delete("/api/profile/delete-account"); localStorage.clear(); window.location.href = "/"; }
        catch { Swal.fire("Error", "Could not delete account. Contact support.", "error"); }
      }
    });
  };
  ...
  return (
    <div className="min-h-screen pb-10" style={{ background: "#f0f9ff" }}>
      ...
      <div
        ...
        style={{
          backgroundImage: `linear-gradient(135deg, rgba(10,35,66,0.92) 0%, rgba(10,35,66,0.70) 50%, rgba(41,171,226,0.80) 100%), url('https://images.unsplash.com/photo-1518770660439-4636190af475?w=1400&q=80')`,
        ...
        <div className="relative z-10 max-w-4xl mx-auto">
          <div className="flex items-start justify-between gap-6 flex-wrap">
            ...
            {/* LEFT */}
            <div>
              <p className="text-blue-200 text-xs font-semibold uppercase tracking-widest mb-1">Account</p>
              <h1 className="text-4xl font-black tracking-tight">Settings ⚙️</h1>
              <p className="text-blue-100 mt-1 text-sm">
                {new Date().toLocaleDateString("en-US", { weekday: "long", month: "long", day: "numeric" })}
              </p>
              <p className="text-blue-200 text-sm mt-1">Manage your account preferences and privacy</p>
            </div>

            {/* RIGHT — info pills */}
            <div className="hidden md:flex gap-2">
              <div className="bg-white/15 backdrop-blur-sm px-4 py-2.5 rounded-xl text-center border border-white/20">
                <p className="text-xs text-blue-200 mb-0.5">Account</p>
                <p className="font-bold text-white text-sm leading-none">{name || "Client"}</p>
              </div>
              <div className="bg-white/15 backdrop-blur-sm px-4 py-2.5 rounded-xl text-center border border-white/20">
                <p className="text-xs text-blue-200 mb-0.5">Role</p>
                <p className="font-bold text-white text-sm leading-none">💪 Client</p>
              </div>
              <div className="bg-white/15 backdrop-blur-sm px-4 py-2.5 rounded-xl text-center border border-white/20">
                <p className="text-xs text-blue-200 mb-0.5">Status</p>
                <p className="font-bold text-white text-sm leading-none">✅ Active</p>
              </div>
            </div>
          </div>
        </div>
        ...
        <div className="flex flex-col sm:flex-row gap-5">
          ...
          <nav className="flex sm:flex-col gap-1 sm:w-52 flex-shrink-0 bg-white rounded-2xl p-3 shadow-sm h-fit">
            ...
            {SECTIONS.map(s => (
              <button key={s.id} onClick={() => setActiveSection(s.id)}
                ...
                onMouseEnter={e => { if (activeSection !== s.id) e.currentTarget.style.background = BLUE_LIGHT; }}
          ...
          <div className="flex-1">
            ...
            {activeSection === "account" && (
              <div className="bg-white rounded-2xl shadow-sm p-6">
                <div className="flex items-center gap-3 mb-6">
                  <div className="w-10 h-10 rounded-xl flex items-center justify-center text-xl" style={{ background: BLUE_LIGHT }}>👤</div>
                  <div>
                ...
                <div className="space-y-6">
                  ...
                  <div>
                    ...
                    <div className="space-y-1">
                      {[
                        { key: "emailWorkoutPlan",        label: "New workout plan assigned",     desc: "When your trainer assigns a new weekly plan"         },
                        { key: "emailMealPlan",            label: "New meal plan assigned",        desc: "When your trainer assigns a nutrition plan"           },
                        { key: "emailSubscriptionUpdate",  label: "Subscription updates",          desc: "Acceptance, rejection, renewal reminders"             },
                        { key: "emailChatMessage",         label: "New chat messages",             desc: "Email digest of unread messages from trainer"         },
                      ].map(item => (
                  ...
                  <div>
                    ...
                    <div className="space-y-1">
                      {[
                        { key: "pushNewMessage",       label: "New messages",        desc: "Badge counter on chat sidebar"                      },
                        { key: "pushPlanUpdate",       label: "Plan updates",        desc: "When trainer edits your workout or meal plan"       },
                        { key: "pushPaymentReminder",  label: "Payment reminders",   desc: "Upcoming subscription renewal alerts"               },
                      ].map(item => (
                        <div key={item.key} className="flex items-center justify-between gap-4 py-3 px-4 rounded-xl hover:bg-gray-50 transition-all">
                          <div>
                            <p className="text-sm font-medium text-gray-700">{item.label}</p>
            ...
            {activeSection === "privacy" && (
              <div className="bg-white rounded-2xl shadow-sm p-6">
                <div className="flex items-center gap-3 mb-6">
                  <div className="w-10 h-10 rounded-xl flex items-center justify-center text-xl" style={{ background: BLUE_LIGHT }}>🛡️</div>
                  <div>
                    <h2 className="text-lg font-bold text-gray-800">Privacy</h2>
                    <p className="text-xs text-gray-400">Control what your trainer can see</p>
                  </div>
                </div>

                <div className="space-y-1 mb-6">
                  {[
                    { key: "showProgressToTrainer", label: "Share weight progress with trainer", desc: "Your trainer can view your weight log charts"         },
                    { key: "showBmiToTrainer",       label: "Share BMI history with trainer",    desc: "Your trainer can view your BMI trend"                 },
                    { key: "allowReviewVisibility",  label: "Show my reviews publicly",          desc: "Your ratings are visible on the public trainer page" },
                  ].map(item => (
                    <div key={item.key} className="flex items-center justify-between gap-4 py-3 px-4 rounded-xl hover:bg-gray-50 transition-all">
                      <div>
                        <p className="text-sm font-medium text-gray-700">{item.label}</p>
                  ...
                  <div className="flex items-start gap-3 mb-4">
                    ...
                    <div>
                      ...
                      <p className="text-xs text-gray-400 mt-0.5 leading-relaxed">
                        Once you delete your account, all your data including progress logs,
                        subscription history, and chat messages will be permanently removed.
