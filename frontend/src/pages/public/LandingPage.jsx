...
export default function LandingPage() {
  ...
  const features = [
    { icon: "📊", title: "Progress Tracking",    desc: "Log weight daily, visualize your journey with charts and get AI-powered goal predictions."    },
    { icon: "🏋️", title: "Weekly Workout Plans", desc: "Personalized day-by-day workout plans with YouTube video guides from your certified trainer." },
    { icon: "🥗", title: "Sri Lankan Nutrition",  desc: "Meal plans built with 57+ local foods. Track calories, protein, carbs and fats daily."        },
    { icon: "💬", title: "Real-Time Chat",        desc: "Chat instantly with your trainer via WebSocket-powered messaging. No refresh needed."         },
    { icon: "⚖️", title: "BMI Calculator",        desc: "Calculate and track your BMI over time with full history and category tracking."              },
    { icon: "💳", title: "Easy Payments",         desc: "Subscribe to trainers with secure Stripe payment and 30-day subscription management."         },
    { icon: "⭐", title: "Trainer Reviews",       desc: "Read real reviews from verified clients and choose the best trainer for your goals."           },
    { icon: "🤖", title: "AI Goal Prediction",    desc: "Machine learning predicts when you'll reach your goal weight based on your progress data."     },
  ];
  ...
  <source src="/fitness.mp4" type="video/mp4" />
        ...
        <div className="absolute inset-0" style={{
          background: "linear-gradient(to top, rgba(0,0,0,0.88) 0%, rgba(0,0,0,0.45) 40%, transparent 70%)"
        }} />

        {/* Content — sits at bottom over gradient */}
        <div className="relative z-10 w-full max-w-7xl mx-auto px-8 md:px-16 pb-16">
          <div className="inline-block px-4 py-1.5 rounded-full text-sm font-semibold mb-5"
            style={{ background: `${BLUE}40`, border: `1px solid ${BLUE}70`, color: "white" }}>
          ...
          <h1 className="text-5xl md:text-7xl font-black leading-tight tracking-tight text-white drop-shadow-lg">
            Achieve Your{" "}
            <span style={{ color: BLUE }}>Fitness Goals</span>
            <br /> with FitTrack
          </h1>
          ...
          <div className="mt-10 flex gap-10 flex-wrap">
            {[
              ...
              <div key={label}>
                <p className="text-3xl font-black drop-shadow" style={{ color: BLUE }}>{value}</p>
      ...
      <section className="px-6 py-20" style={{ background: "#f0f9ff" }}>
        <div className="max-w-5xl mx-auto">
          <div className="text-center mb-14">
            <h2 className="text-4xl font-black text-gray-900">How FitTrack Works</h2>
            <p className="text-gray-500 mt-3 text-lg">Get started in 4 simple steps</p>
          </div>
          <div className="grid md:grid-cols-4 gap-6">
            {[
              { step: "01", icon: "📝", title: "Register",       desc: "Create your free account in minutes"                   },
              { step: "02", icon: "🔍", title: "Find Trainer",   desc: "Browse verified trainers and read real reviews"        },
              { step: "03", icon: "💪", title: "Get Your Plan",  desc: "Receive personalized workout & meal plans"             },
              { step: "04", icon: "📈", title: "Track Progress", desc: "Log daily and watch your transformation unfold"        },
              ...
              <div key={step} className="text-center relative">
                ...
                <div className="absolute top-0 right-4 w-7 h-7 rounded-full text-white text-xs font-black flex items-center justify-center"
                  style={{ background: BLUE }}>
                  {step}
                </div>
      ...
      <section className="px-6 py-20 bg-white">
        <div className="max-w-6xl mx-auto">
          <div className="text-center mb-14">
            ...
            <p className="text-gray-500 mt-3 text-lg">Certified professionals ready to guide you</p>
          </div>
          ...
          {loading ? (
            ...
            <div className="grid md:grid-cols-3 gap-6">
              {(trainers.length > 0 ? trainers : [
                { name: "Kasun Perera",   specialization: "Weight Loss",        experienceYears: 5, pricePerMonth: 3500, userId: 1 },
                { name: "Nimali Silva",   specialization: "Muscle Gain",        experienceYears: 7, pricePerMonth: 4000, userId: 2 },
                { name: "Ruwan Fernando", specialization: "Yoga & Flexibility",  experienceYears: 4, pricePerMonth: 3000, userId: 3 },
              ]).map(t => (
                <div key={t.userId}
                  ...
                  <div className="p-6 text-center"
                    style={{ background: `linear-gradient(135deg, ${BLUE_DARK}15, ${BLUE}15)` }}>
                    <div className="w-20 h-20 rounded-full mx-auto mb-3 text-white font-black text-3xl flex items-center justify-center"
                      style={{ background: `linear-gradient(135deg, ${BLUE_DARK}, ${BLUE})` }}>
                      {t.name?.charAt(0)}
                    </div>
                    <h3 className="text-lg font-black text-gray-800">{t.name}</h3>
                    <p className="text-sm mt-1 font-bold" style={{ color: BLUE }}>
                  ...
                  <div className="p-5 space-y-2 bg-white">
                    {t.specialization  && <p className="text-sm text-gray-600">🎯 {t.specialization}</p>}
                    {t.experienceYears && <p className="text-sm text-gray-600">📅 {t.experienceYears} years experience</p>}
                    {t.bio             && <p className="text-xs text-gray-400 italic line-clamp-2">"{t.bio}"</p>}
                    <button
                      onClick={() => navigate(isLoggedIn ? getDashboard() : "/register")}
        ...
        <div className="max-w-6xl mx-auto">
          <div className="text-center mb-14">
            ...
            <p className="text-blue-200 mt-3 text-lg">
              {reviews.length > 0 ? "Real reviews from real FitTrack clients" : "Real results from real people"}
            </p>
          </div>

          <div className="grid md:grid-cols-3 gap-6">
            {(reviews.length > 0 ? reviews : [
              { clientName: "Amal K.",     rating: 5, review: "FitTrack helped me stay consistent. Lost 12kg in 4 months!",          trainerName: "Kasun Perera"   },
              { clientName: "Shashika D.", rating: 5, review: "My trainer customized workouts perfectly. Gained muscle and confidence!", trainerName: "Nimali Silva"   },
              { clientName: "Nuwan P.",    rating: 4, review: "Easy to follow plans and great progress tracking. Highly recommended!",  trainerName: "Ruwan Fernando" },
            ]).map((r, i) => (
              <div key={i} className="rounded-2xl p-6 border border-white/20"
                style={{ background: "rgba(255,255,255,0.08)", backdropFilter: "blur(10px)" }}>
                <div className="flex items-center gap-1 mb-3">
                  {renderStars(r.rating)}
                  <span className="text-yellow-400 text-sm font-bold ml-1">{r.rating}.0</span>
                </div>
                {r.review && (
                  <p className="text-blue-100 text-sm leading-relaxed mb-4">"{r.review}"</p>
                )}
                <div className="border-t border-white/20 pt-3">
                  <p className="text-white font-bold text-sm">— {r.clientName}</p>
                  <p className="text-blue-300 text-xs mt-1">Review for {r.trainerName}</p>
                </div>
              </div>
            ))}
          </div>
      ...
      <section className="px-6 py-20 text-white text-center"
        ...
        <div className="max-w-2xl mx-auto">
          <h2 className="text-4xl font-black mb-4">
            {isLoggedIn ? "Continue Your Journey!" : "Ready to Transform Your Body?"}
          </h2>
