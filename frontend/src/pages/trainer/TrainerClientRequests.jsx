...
export default function TrainerClientRequests() {
  ...
  const handleAccept = async (id, clientName) => {
    Swal.fire({
      ...
      html: `<b>${clientName}</b> will become your client.<br>They will need to complete payment to activate the subscription.`,
    ...
    Swal.fire({
      ...
      html: `
        <p style="color:#6b7280;margin-bottom:12px;font-size:14px;">
          Please provide a reason for rejecting <b style="color:#111">${clientName}</b>. This will be shown to the client.
        </p>
        <textarea id="rejectReason"
          ...
          rows="3" placeholder="e.g. I am fully booked at the moment. Please try again next month."></textarea>
          ...
          <div className="flex items-start justify-between gap-6 flex-wrap">
            ...
            <div className="hidden md:block">
              <div className="bg-white/15 backdrop-blur-sm px-5 py-3 rounded-2xl border border-white/20 text-center">
                ...
                <p className="text-xs text-teal-200 mt-1">request{requests.length !== 1 ? "s" : ""}</p>
              </div>
            </div>
          </div>
        </div>
      ...
      <div className="max-w-6xl mx-auto px-4 py-6">
        {requests.length === 0 ? (
          <div className="bg-white rounded-2xl shadow-sm p-14 text-center">
            ...
            <p className="text-gray-400 text-sm mt-2">New client requests will appear here</p>
          </div>
        ) : (
          <div className="space-y-5">
            {requests.map(req => {
              ...
              return (
                <div key={req.id} className="bg-white rounded-2xl shadow-sm border-2 overflow-hidden transition-all"
                  ...
                  <div className="p-6">
                      ...
                      <div className="flex-1">
                        ...
                        {bmi && bmiInfo && (
                          <div className="mt-2 flex items-center gap-2">
                            <div className="flex h-1.5 rounded-full overflow-hidden gap-0.5 w-32">
                              {[{ color: "#3b82f6", w: "25%" }, { color: "#22c55e", w: "25%" }, { color: "#f97316", w: "25%" }, { color: "#ef4444", w: "25%" }].map((b, i) => (
                                <div key={i} className="h-full rounded-sm" style={{ width: b.w, background: b.color }} />
                              ))}
                            </div>
                            <span className="text-xs font-medium" style={{ color: bmiInfo.color }}>{bmiInfo.label}</span>
                          </div>
                        )}
                    ...
                    {isExpanded && client && (
                      <div className="mt-5 pt-4 border-t border-gray-100">
                        ...
                        {(!client.heightCm || !client.weightKg || !client.age) && (
                          <div className="mt-4 p-3 rounded-xl text-sm" style={{ background: "#fef3c7", color: "#92400e" }}>
                            ⚠️ This client has an incomplete profile. Some health information is missing.
                          </div>
                        )}
                      </div>
                    )}
                  ...
                  <div className="px-6 py-4 border-t border-gray-100 flex gap-3 justify-end"
                    ...
                    <button onClick={() => handleReject(req.id, req.clientName)}
                      className="px-6 py-2.5 rounded-xl text-white font-semibold text-sm hover:opacity-90 transition-all active:scale-95"
