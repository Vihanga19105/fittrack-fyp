...
export default function TrainerClientProgress() {
  ...
  const selectClient = async (client) => {
    ...
    try {
      ...
      const wLogs = [...(wRes.data || [])].sort((a, b) =>
        new Date(a.loggedAt || a.logDate) - new Date(b.loggedAt || b.logDate));
      const bLogs = bRes.data || [];
      const profile = profileRes.data || {};
      const byDate = workoutRes.data || {};
      const totalCount = Object.values(byDate).reduce((s, v) => s + Number(v), 0);
      ...
      setBadges(calculateBadges({
        weightLogs: wLogs, workoutCompletions: totalCount,
        bmiLogs: bLogs, goalWeight: profile.goalWeight,
        subscriptionStartDate: client.startDate,
      }));
    } catch {}
  ...
  const weightChartData = weightLogs.map(log => ({
    ...
    weight: log.weightKg,
    goal: clientProfile?.goalWeight || null,
  }));
      ...
      <div className="max-w-6xl mx-auto px-4 py-6">
        {clients.length === 0 ? (
          <div className="bg-white rounded-2xl shadow-sm p-12 text-center">
            ...
            <p className="text-gray-400 text-sm mt-2">Accept client requests to monitor progress</p>
          </div>
        ) : (
          <div className="grid grid-cols-1 lg:grid-cols-4 gap-6">
            ...
            <div className="bg-white rounded-2xl shadow-sm p-4">
              <h3 className="font-bold text-gray-500 mb-4 text-xs uppercase tracking-wide">
                    ...
                    <div className="mt-4 pt-4 border-t border-gray-100">
                      <div className="flex items-center justify-between mb-2">
                        <p className="text-xs font-bold uppercase tracking-wide text-gray-400">
                          Achievements ({earnedBadgesCount}/{badges.length})
                        </p>
                      </div>
                        ...
                        <div className="p-6">
                          ...
                          {activeTab === "weight" && (
                            <>
                              <div className="flex justify-between items-center mb-4">
                                <div>
                                  <h3 className="font-bold text-gray-800">Weight Progress</h3>
                                  <p className="text-xs text-gray-400">{weightChartData.length} entries logged</p>
                                </div>
                                {weightChange && (
                                  <span className="px-3 py-1 rounded-full text-xs font-bold text-white"
                                    style={{ background: weightChange < 0 ? "#10b981" : "#ef4444" }}>
                                    {weightChange > 0 ? "+" : ""}{weightChange} kg
                                  </span>
                                )}
                              </div>
                              {weightChartData.length < 2 ? (
                                <div className="text-center py-12 rounded-xl" style={{ background: TEAL_LIGHT }}>
                                  <p className="text-3xl mb-2">⚖️</p>
                                  <p className="text-sm" style={{ color: TEAL_DARK }}>No weight logs yet</p>
                                </div>
                              ) : (
                                <ResponsiveContainer width="100%" height={250}>
                                  <AreaChart data={weightChartData}>
                                    ...
                                    <Tooltip contentStyle={{ borderRadius: "12px", border: "none", boxShadow: "0 4px 20px rgba(0,0,0,0.1)" }}
                                      formatter={(v, n) => [`${v} kg`, n === "weight" ? "Weight" : "Goal"]} />
                                    <Area type="monotone" dataKey="weight" stroke={TEAL} strokeWidth={2.5} fill="url(#wGrad)"
                                      dot={{ fill: TEAL, r: 4 }} activeDot={{ r: 6 }} name="weight" />
                                    {goalWeight && (
                                      <ReferenceLine y={goalWeight} stroke="#22c55e" strokeDasharray="6 4"
                                        label={{ value: `Goal: ${goalWeight}kg`, fill: "#22c55e", fontSize: 11 }} />
                                    )}
                                  </AreaChart>
                                </ResponsiveContainer>
                              )}
                          ...
                          {activeTab === "bmi" && (
                            <>
                              <div className="flex justify-between items-center mb-4">
                                <div>
                                  <h3 className="font-bold text-gray-800">BMI History</h3>
                                  <p className="text-xs text-gray-400">{bmiChartData.length} measurements</p>
                                </div>
                                {latestBmi && (
                                  <span className="px-3 py-1 rounded-full text-xs font-bold text-white"
                                    style={{ background: getBmiColor(latestBmi) }}>
                                    {getBmiLabel(latestBmi)}
                                  </span>
                                )}
                              </div>
                              {bmiChartData.length < 2 ? (
                                <div className="text-center py-12 rounded-xl" style={{ background: TEAL_LIGHT }}>
                                  ...
                                  <p className="text-sm" style={{ color: TEAL_DARK }}>No BMI logs yet</p>
                                </div>
                              ) : (
                                <>
                                  <ResponsiveContainer width="100%" height={220}>
                                    <LineChart data={bmiChartData}>
                                      ...
                                      <Line type="monotone" dataKey="bmi" stroke={TEAL} strokeWidth={2.5}
                                  ...
                                  <div className="grid grid-cols-4 gap-2 mt-4">
                                    {[
                                      ...
                                      { label: "Overweight",  range: "25-30",   color: "#f97316" },
                          ...
                          {activeTab === "workout" && (
                            <>
                              <div className="flex justify-between items-center mb-4">
                                <div>
                                  <h3 className="font-bold text-gray-800">Workout Activity</h3>
                                  <p className="text-xs text-gray-400">Last 7 days</p>
                                </div>
                                <span className="px-3 py-1 rounded-full text-xs font-bold text-white" style={{ background: "#8b5cf6" }}>
                                  {totalWorkouts} total
                                </span>
                              </div>
                          ...
                          {activeTab === "badges" && (
                            <>
                              <div className="flex justify-between items-center mb-4">
                                <div>
                                  <h3 className="font-bold text-gray-800">Achievement Badges</h3>
                                  <p className="text-xs text-gray-400">{earnedBadgesCount} of {badges.length} earned</p>
                                </div>
                                <span className="text-2xl">🏆</span>
                              </div>
                              <div className="h-2 bg-gray-100 rounded-full overflow-hidden mb-6">
                                <div className="h-full rounded-full"
                                  style={{ width: `${(earnedBadgesCount / badges.length) * 100}%`, background: `linear-gradient(90deg, #f59e0b, ${TEAL})` }} />
                              </div>
