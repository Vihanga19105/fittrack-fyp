...
export default function TrainerIncomeReport() {
  ...
  if (loading) return (
    <div className="flex items-center justify-center min-h-screen" style={{ background: "#f0fdf4" }}>
      <div className="text-center"><div className="text-4xl animate-bounce mb-3">💰</div><p className="text-gray-400 animate-pulse">Loading income data...</p></div>
    </div>
  );
      ...
      <div className="relative text-white px-8 py-12 overflow-hidden"
        ...
        <div className="relative z-10 max-w-6xl mx-auto flex items-start justify-between gap-6 flex-wrap">
          ...
          <div className="hidden md:flex gap-2">
            {[
              { label: "This Month", value: `LKR ${totalIncome.toLocaleString()}` },
              { label: "All Time",   value: `LKR ${totalEarned.toLocaleString()}` },
              { label: "Per Client", value: `LKR ${price.toLocaleString()}`       },
            ].map(p => (
              <div key={p.label} className="bg-white/15 backdrop-blur-sm px-4 py-2.5 rounded-xl text-center border border-white/20">
                <p className="text-xs text-teal-200 mb-0.5">{p.label}</p>
                <p className="font-black text-white text-lg leading-none">{p.value}</p>
              </div>
            ))}
          </div>
        </div>
      ...
      <div className="max-w-6xl mx-auto px-4 py-6 space-y-5">
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
          {[
            ...
            <div key={label} className="bg-white rounded-2xl shadow-sm p-5 flex items-center gap-3">
              ...
              <div><p className="text-xs text-gray-500">{label}</p><p className="text-lg font-bold text-gray-800 leading-tight">{value}</p></div>
            </div>
          ))}
        </div>
        ...
        <div className="bg-white rounded-2xl shadow-sm overflow-hidden">
          <div className="flex border-b border-gray-100">
            {[{ id: "overview", label: "📊 Overview" }, { id: "clients", label: "👥 Active Clients" }, { id: "history", label: "📋 Payment History" }].map(tab => (
          ...
          <div className="p-6">
                    ...
                    <div>
                      ...
                      <ResponsiveContainer width="100%" height={220}>
                        <AreaChart data={monthlyData}>
                          <defs><linearGradient id="incGrad" x1="0" y1="0" x2="0" y2="1"><stop offset="5%" stopColor={TEAL} stopOpacity={0.3} /><stop offset="95%" stopColor={TEAL} stopOpacity={0} /></linearGradient></defs>
                          <CartesianGrid strokeDasharray="3 3" stroke="#f0f0f0" />
                          <XAxis dataKey="month" tick={{ fontSize: 11, fill: "#9ca3af" }} />
                          <YAxis tick={{ fontSize: 11, fill: "#9ca3af" }} />
                          <Tooltip contentStyle={{ borderRadius: "12px", border: "none", boxShadow: "0 4px 20px rgba(0,0,0,0.1)" }} formatter={v => [`LKR ${v.toLocaleString()}`, "Income"]} />
                          <Area type="monotone" dataKey="income" stroke={TEAL} strokeWidth={2.5} fill="url(#incGrad)" dot={{ fill: TEAL, r: 4 }} activeDot={{ r: 6 }} />
                        </AreaChart>
                      </ResponsiveContainer>
                    </div>
                    <div>
                      ...
                      <ResponsiveContainer width="100%" height={200}>
                        <BarChart data={monthlyData}>
                          ...
                          <XAxis dataKey="month" tick={{ fontSize: 11, fill: "#9ca3af" }} />
                          <YAxis tick={{ fontSize: 11, fill: "#9ca3af" }} />
                          <Tooltip contentStyle={{ borderRadius: "12px", border: "none" }} formatter={v => [`${v} clients`, "Clients"]} />
            ...
            {activeTab === "clients" && (
              <>
                {activeSubs.length === 0 ? (
                  <div className="text-center py-16"><p className="text-5xl mb-4">👥</p><p className="text-gray-500 font-semibold text-lg">No active clients yet</p></div>
                ) : (
                    ...
                    <table className="w-full text-sm">
                      <thead>
                        <tr style={{ background: TEAL_LIGHT }}>
                          {["Client", "Email", "Since", "Expires", "Days Left", "Monthly Fee"].map(h => (
                            <th key={h} className="text-left px-4 py-3 text-xs font-bold uppercase tracking-wide" style={{ color: TEAL_DARK }}>{h}</th>
                          ))}
                        </tr>
                      </thead>
                      <tbody className="divide-y divide-gray-50">
                        {activeSubs.map((c, i) => {
                          ...
                          return (
                            <tr key={c.id} className="hover:bg-gray-50 transition" style={{ background: i % 2 === 0 ? "white" : "#fafafa" }}>
                              <td className="px-4 py-4">
                                <div className="flex items-center gap-3">
                                  {/* ✅ Client photo in active clients table */}
                                  {c.clientProfileImage ? (
                                    <img src={c.clientProfileImage} alt={c.clientName}
                                      className="w-8 h-8 rounded-full object-cover flex-shrink-0 border" style={{ borderColor: TEAL }} />
                                  ) : (
                                    <div className="w-8 h-8 rounded-full text-white font-bold text-xs flex items-center justify-center" style={{ background: TEAL }}>
                              ...
                              <td className="px-4 py-4">
                                {days !== null ? (
                                  <span className="font-bold text-sm" style={{ color: isExpiring ? "#f59e0b" : TEAL }}>{isExpiring ? `⚠ ${days}d` : `${days}d`}</span>
                                ) : <span className="text-gray-300">—</span>}
                              </td>
                              <td className="px-4 py-4 font-bold" style={{ color: "#10b981" }}>LKR {price.toLocaleString()}</td>
                            </tr>
                          );
                        })}
                      </tbody>
                      <tfoot>
                        <tr style={{ background: TEAL_LIGHT }}>
                          <td colSpan={5} className="px-4 py-3 font-bold text-gray-700">Total Monthly Income</td>
                          <td className="px-4 py-3 font-black text-xl" style={{ color: TEAL }}>LKR {totalIncome.toLocaleString()}</td>
                        </tr>
                      </tfoot>
                    </table>
                  </div>
            ...
            {activeTab === "history" && (
              <>
                {allSubs.length === 0 ? (
                  <div className="text-center py-16"><p className="text-5xl mb-4">📋</p><p className="text-gray-500 font-semibold text-lg">No payment history yet</p></div>
                ) : (
                  <div className="overflow-x-auto">
                    <table className="w-full text-sm">
                      <thead>
                      ...
                      <tbody className="divide-y divide-gray-50">
                        {allSubs.map((sub, i) => (
                          <tr key={sub.id} className="hover:bg-gray-50 transition" style={{ background: i % 2 === 0 ? "white" : "#fafafa" }}>
                            <td className="px-4 py-4">
                              <div className="flex items-center gap-3">
                                {/* ✅ Client photo in history table */}
                            ...
                            <td className="px-4 py-4 font-bold" style={{ color: (sub.status === "ACTIVE" || sub.status === "EXPIRED") ? "#10b981" : "#9ca3af" }}>
                              {(sub.status === "ACTIVE" || sub.status === "EXPIRED") ? `+LKR ${price.toLocaleString()}` : "—"}
                            </td>
                            <td className="px-4 py-4">
                      ...
                      <tfoot>
                        <tr style={{ background: TEAL_LIGHT }}>
                          ...
                          <td colSpan={2} className="px-4 py-3 font-black text-xl" style={{ color: TEAL }}>LKR {totalEarned.toLocaleString()}</td>
                        </tr>
                      </tfoot>
                    </table>
                  </div>
