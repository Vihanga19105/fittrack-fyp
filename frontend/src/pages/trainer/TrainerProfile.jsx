...
export default function TrainerProfile() {
    ...
    api
      ...
      .then((res) => {
        const data = {
          ...
          // Supports both array style and old string style from backend
          specialization: Array.isArray(res.data.specialization)
            ? res.data.specialization
            : res.data.specialization
            ? res.data.specialization.split(",").map((s) => s.trim())
            : [],
  ...
  const validate = () => {
    ...
    if (
      ...
      isNaN(profile.experienceYears) ||
      Number(profile.experienceYears) < 0 ||
      Number(profile.experienceYears) > 50
    ) {
    ...
    if (
      ...
      e.specialization = "Select at least one specialization";
    }

    if (
      !profile.pricePerMonth ||
      isNaN(profile.pricePerMonth) ||
      Number(profile.pricePerMonth) <= 0 ||
      Number(profile.pricePerMonth) > 100000
    ) {
  ...
  const handleSave = async () => {
    if (!validate()) {
      Swal.fire("Fix Errors", "Please fix all errors before saving", "warning");
      return;
    }
    ...
    try {
      ...
      await api.put("/api/profile/trainer", {
        ...
        // Send as string to match existing backend column
  ...
  const filled = fields.filter((f) => {
    ...
    return profile[f] !== "" && profile[f] !== null;
  }).length;
            ...
            <div className="flex items-center gap-5">
              <div className="relative flex-shrink-0">
                ...
                {isEditing && (
                  ...
                  >
                    <span style={{ color: TEAL }} className="text-sm">
                      📷
                    </span>
                  </button>
                )}
                  ...
                  {Array.isArray(profile.specialization) &&
                    profile.specialization.length > 0 && (
                      <span className="text-teal-200 text-xs">
                        • {profile.specialization.slice(0, 3).join(", ")}
              ...
              <div className="hidden md:flex gap-2">
                ...
                <div className="bg-white/15 backdrop-blur-sm px-4 py-2.5 rounded-xl text-center border border-white/20">
                  ...
                  <p className="font-bold text-white text-sm leading-none">
                    {profile.pricePerMonth
                      ? `LKR ${Number(profile.pricePerMonth).toLocaleString()}`
                      : "--"}
                  </p>
                </div>
            ...
            {isEditing ? (
              <>
                <textarea
                  ...
                  className={`w-full border rounded-xl px-4 py-3 text-gray-800 text-sm focus:outline-none resize-none transition-all ${
                    errors.bio
                  ...
                  onBlur={(e) =>
                    (e.target.style.borderColor = errors.bio
                      ? "#f87171"
                      : "#f3f4f6")
                  }
                />
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              ...
              {isEditing ? (
                <>
                  <input
                    ...
                    className={`w-full border rounded-xl px-4 py-3 text-gray-800 text-sm focus:outline-none transition-all ${
                      errors.name
                        ? "border-red-400 bg-red-50"
                        : "border-gray-200 bg-gray-50"
                    }`}
            ...
            <div>
              ...
              {isEditing ? (
                <>
                  <div
                    className={`border rounded-xl p-3 bg-gray-50 max-h-56 overflow-y-auto ${
                      ...
                      {SPECIALIZATION_OPTIONS.map((s) => (
                        <label
                          ...
                          className={`flex items-center gap-2 px-3 py-2 rounded-lg cursor-pointer text-sm transition-all ${
                            profile.specialization.includes(s)
                        ...
                        >
                          <input
                            type="checkbox"
                            checked={profile.specialization.includes(s)}
                            onChange={() => handleSpecializationToggle(s)}
              ...
              {isEditing ? (
                <>
                  <input
                    ...
                    onFocus={(e) => (e.target.style.borderColor = TEAL)}
                    onBlur={(e) =>
                      (e.target.style.borderColor = errors.certification
                        ? "#f87171"
                        : "#f3f4f6")
                    }
                  />
              ...
              {isEditing ? (
                <>
                  <input
                    ...
                    onFocus={(e) => (e.target.style.borderColor = TEAL)}
                    onBlur={(e) =>
                      (e.target.style.borderColor = errors.experienceYears
                        ? "#f87171"
                        : "#f3f4f6")
                    }
                  />
                ...
                <>
                  <div className="relative">
                    <input
                      type="number"
                      name="pricePerMonth"
                      value={profile.pricePerMonth}
                      onChange={handleChange}
                      placeholder="e.g. 5000"
                      min="0"
                      max="100000"
                      className={`w-full border rounded-xl px-4 py-3 text-gray-800 text-sm focus:outline-none transition-all pr-16 ${
                      ...
                      onFocus={(e) => (e.target.style.borderColor = TEAL)}
                      onBlur={(e) =>
                        (e.target.style.borderColor = errors.pricePerMonth
                          ? "#f87171"
                          : "#f3f4f6")
                      }
                    />
        ...
        <div className="bg-white rounded-2xl shadow-sm p-6">
          <div className="flex justify-between items-center mb-3">
            ...
            <span
              ...
              style={{ color: pct === 100 ? TEAL : "#f59e0b" }}
            >
              {pct}%
            </span>
          </div>
            ...
            <div
              ...
              style={{
                ...
                background:
                  pct === 100
                    ? TEAL
                    : `linear-gradient(90deg, ${TEAL_DARK}, ${TEAL})`,
              }}
            />
          </div>
